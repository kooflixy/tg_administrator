...
class ModeratorChatORMHandler(BaseORMHandler[ModeratorChatORM]):
    ...
    async def get_all_moderator_ids_in_chat(cls, session: AsyncSession, chat_id: int):
        query = select(ModeratorChatORM.moderator_id).filter(
            ModeratorChatORM.chat_id == chat_id
        )
    ...
    @classmethod
    async def get_by_moderator_and_chat_ids(
        cls, session: AsyncSession, moderator_id: int, chat_id: int
    ) -> Optional[ModeratorChatORM]:
        """Получает одну определенную запись по pk_value"""
        query = (
            select(cls.model_cls)
            .filter(
                cls.model_cls.moderator_id == moderator_id,
                cls.model_cls.chat_id == chat_id,
            )
            .options(
                selectinload(cls.model_cls.chat), selectinload(cls.model_cls.moderator)
            )
        )
    ...
    @classmethod
    async def remove_by_moderator_and_chat_ids(
        cls, session: AsyncSession, moderator_id: int, chat_id: int
    ) -> None:
        ...
        query = delete(cls.model_cls).filter(
            cls.model_cls.moderator_id == moderator_id, cls.model_cls.chat_id == chat_id
        )
    ...
    ) -> list[Optional[ModeratorChatORM]]:
        if not isinstance(moderator_id, int):
            raise TypeError("moderator_id должен быть int")
    ...
    async def is_last_page(cls, session, page: int, moderator_id: int) -> bool:
        ...
        query = text(
            f"SELECT COUNT(*) FROM {cls.model_cls.__tablename__} WHERE moderator_id={moderator_id}"
        )
    ...
    async def change_permission(
        cls, session: AsyncSession, moderator_id: int, chat_id: int, perm_name: str
    ) -> None:
        if not isinstance(moderator_id, int):
            raise TypeError("moderator_id должен быть int")
