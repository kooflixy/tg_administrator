...
class BaseORMHandler(Generic[ModelType], ABC):
    ...
    def _excert_page_result(query, page: int, page_objs_count: int):
        ...
        query = query.offset(page_objs_count * (page - 1)).limit(page_objs_count)
        return query

    @classmethod
    async def _get_page(
        cls, session: AsyncSession, page: int, query
        ...
        query = cls._excert_page_result(
            query, page, changeable_settings.max_count_in_page
        )
    ...
    async def get_page(
        ...
        query = select(cls.model_cls).order_by(
            desc(cls.model_cls.created_at), desc(cls.model_cls.id)
        )
