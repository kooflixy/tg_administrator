...
@router.callback_query(SettingsTypeCD.filter(F.type == SettingsType.LOCAL))
async def get_local_settings(callback: CallbackQuery, callback_data: SettingsTypeCD):
    if changeable_settings.local_chat_id:
        id_ = str(changeable_settings.local_chat_id)
        if id_[:4] == "-100":
            id_ = id_[4:]
        elif id_[:1] == "-":
            id_ = id_[1:]
...
<b>ID:</b> {id_}"""
        ...
        text = "Чат для локала еще не привязан"

    text += "\n\n<code>/set_local_chat</code> <b>[айди или ссылка на чат]</b> - изменение чата для локала"
...
@router.message(Command("set_local_chat"), IsAdminChat())
async def set_local_chat(message: Message, command: CommandObject):
    ...
    if not chat:
        ...
        log.info(
            "%s ввёл несуществующую ссылку на local чат chat_url=%r",
    ...
    if isinstance(chat, types.Channel):
        if not chat.megagroup:
            ...
            log.info(
                "%s ввёл ссылку на локал чат, но это оказался не чат, а канал chat_url=%r",
                name_in_log.user(message),
                message.text,
            )
            return
        changeable_settings.local_chat_id = int("-100" + str(chat.id))
    elif isinstance(chat, types.Chat):
        changeable_settings.local_chat_id = int("-" + str(chat.id))
    else:
        await message.answer("Это не чат")
        return
    await message.answer(f"Чат для локала успешно изменен на <b>{chat.title}</b>")

    log.info(
        "id local-чата изменено local_chat_id=%s", changeable_settings.local_chat_id
    )
