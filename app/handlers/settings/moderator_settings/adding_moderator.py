...
async def add_moderator(callback: CallbackQuery, callback_data: AddModeratorCD, state: FSMContext):
    ...
    log.info('Пользователя %s попросили ввести юз или айди пользователя для добавления его в модераторы', name_in_log.user(callback))
...
async def input_moderator_username(message: Message, state: FSMContext):
    ...
    log.debug('%s ввёл юзернейм пользоваетля для его добавления в модераторы moderator_username=%r', name_in_log.user(message), message.text)
    ...
    try:
        ...
        log.exception('При попытке получить пользователя с username=%r произошла ошибка', message.text)
        return
    
    if not user:
        await message.answer('Добавить модератора не удалось, попробуйте еще раз')
        log.info('%s ввёл несуществующий юзернейм пользователя для его добавления в отслеживаемые moderator_username=%r', name_in_log.user(message), message.text)
        return

    # Проверка, на тип: является ли пользователем
    if not isinstance(user, types.User):
        await message.answer('Это не пользователь')
        log.info('%s ввёл юзернейм пользователя для его добавления в отслеживаемые, но это оказался не пользователь moderator_username=%r type=%r', name_in_log.user(message), message.text, type(user))
        return
    ...
    try:
        async with async_session_factory() as session:
            ...
            if user.id in await ModeratorORMHandler.get_all_id(session):
                ...
                log.info('%s ввёл юзернейм пользователя для его добавления в модераторы, но он уже является им moderator_username=%r moderator_id=%s', name_in_log.user(message), message.text, user.id)
                return
        ...
        await message.answer('⚠Произошла ошибка')
        log.exception('При попытке добавить модератора произошла ошибка moderator_username=%r moderator_id=%s', message.text, user.id)
    
    await message.answer(f'Пользователь {TextMarkup.tag_user(user_full_name, user.id)} теперь модератор✅')
    log.info('%s добавил пользователя в модераторы moderator_username=%s moderator_id=%s', name_in_log.user(message), message.text, user.id)
