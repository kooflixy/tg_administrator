...
async def add_chat(callback: CallbackQuery, callback_data: AddChatCD, state: FSMContext):
    ...
    log.info('Пользователя %s попросили ввести ссылку или айди чата', name_in_log.user(callback))
...
async def input_chat_url(message: Message, state: FSMContext):
    ...
    log.debug('%s ввёл ссылку на чат для его добавления в отслеживаемые chat_url=%r', name_in_log.user(message), message.text)
    ...
    try:
        ...
        log.exception('При попытке получить чата с chat_url=%r произошла ошибка', message.text)
        return
    
    if not chat:
        await message.answer('Добавить чат не удалось, попробуйте еще раз')
        log.info('%s ввёл несуществующую ссылку на чат для его добавления в отслеживаемые chat_url=%r', name_in_log.user(message), message.text)
        return
    ...
    if not isinstance(chat, types.Channel):
        ...
        log.info('%s ввёл ссылку на чат для его добавления в отслеживаемые, но это оказался не чат chat_url=%r', name_in_log.user(message), message.text)
        return
    ...
    try:
        ...
        await message.answer('⚠Произошла ошибка')
        log.exception('При попытке добавить чат с chat_url=%r произошла ошибка', message.text)
        return
    ...
    log.info('%s добавил чат в отслеживаемые chat_url=%r', name_in_log.user(message), message.text)