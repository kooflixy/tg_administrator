...
async def captcha_settings(callback: CallbackQuery, callback_data: SettingsTypeCD):
    log.debug('%s заросил настройки капчи',
...
<b>🔔Текст кнопки капчи:</b> {changeable_settings.captcha_button_text}''',
    ...
    log.info('%s получил настройки капчи',
                name_in_log.user(callback))
...
async def change_captcha_status(callback: CallbackQuery, callback_data: ChangeCaptchaStatusCD):
    ...
    log.info('%s получил изменил статус капчи на %s',
                name_in_log.user(callback), changeable_settings.captcha_status)
...
async def change_captcha_waiting_time(callback: CallbackQuery, callback_data: ChangeCaptchaWaitingTimeCD):
    ...
    log.info('%s изменил время ожидания прохождения капчи на %s секунд',
                name_in_log.user(callback), changeable_settings.captcha_waitng)
...
async def set_captcha_text(message: Message, state: FSMContext):
    ...
    log.info('%s изменил текст %s на %r',
                name_in_log.user(message), data['type'], message.text)
