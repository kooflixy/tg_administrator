...
async def mute_user(message: Message, command: CommandObject):
    ...
    if not await KickRestHandler.is_perm_exists(message.from_user.id, message.chat.id): return
    ...
    await RestChecker.reply_n_delete(f'{TextMarkup.tag_user(user.name, user.id)} успешно кикнут', message)