...
def chat_list_ikb(chat_list: list[ChatORM], page: int, is_last_page: bool):
    ...
    for chat in chat_list:
        builder.button(text=chat.name, callback_data=ChatDetailsCD(back_page=page, chat_id=chat.id).pack())
    builder.attach(get_paginator_ikb(ChatListCD, cur_page=page, is_last_page=is_last_page))
...
def chat_details_ikb(chat_id: int, back_page: int):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text='❌Удалить', callback_data=RemoveChatCD(chat_id=chat_id, back_page=back_page).pack())
