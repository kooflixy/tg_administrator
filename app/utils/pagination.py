...
def get_paginator_ikb(callback_data_cls: CallbackData, *, cur_page: int = 1, is_last_page: bool, **kwargs):
    builder = InlineKeyboardBuilder()

    if cur_page<=1:
        builder.button(text='⏪', callback_data=FirstPageErrCD().pack())
    else:
        builder.button(text='⏪', callback_data=callback_data_cls(cur_page=cur_page-1, **kwargs).pack())
